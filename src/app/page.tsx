...
interface GalleryImage {
  ...
  width?: number;
  height?: number;
  // Grid layout fields for custom bento grid positioning (from admin)
  displayOrder?: number;
  gridRow?: number;
  gridColumn?: number;
  gridSpan?: number;
...
function HomeContent() {
  ...
  // Helper to create a URL-safe ID from show date and venue
  const getShowId = (show: { date: string; venue: string }) => {
    const dateSlug = show.date.split(",")[0].toLowerCase().replace(/\s+/g, "-");
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(
              upcomingShows.map((show) => ({
                ...
                "@type": "MusicEvent",
                name: `The Dutch Queen Unplugged live at ${show.venue}`,
                startDate: show.date,
                location: {
                  "@type": "Place",
                  name: show.venue,
